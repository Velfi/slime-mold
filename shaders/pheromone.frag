...
void main() {
    ...
    // Apply diffusion (simple box blur)
    vec2 texel_size = 1.0 / textureSize(t_pheromone, 0);
    vec4 neighbors = vec4(0.0);
    neighbors += texture(t_pheromone, v_tex_coords + vec2(-1, -1) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2(-1,  0) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2(-1,  1) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2( 0, -1) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2( 0,  1) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2( 1, -1) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2( 1,  0) * texel_size);
    neighbors += texture(t_pheromone, v_tex_coords + vec2( 1,  1) * texel_size);
    ...
    // Mix current value with diffused value
    float diffusion = pheromone_diffusion_rate * delta_time;
    current.r = mix(current.r, neighbors.r, diffusion);
    
    // Add new deposition
    current.r += deposition.r;
